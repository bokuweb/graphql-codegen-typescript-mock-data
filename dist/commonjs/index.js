...
const indefinite_1 = (0, tslib_1.__importDefault)(require("indefinite"));
...
const createNameConverter = (convention, transformUnderscore) => (value, prefix = '') => {
    switch (convention) {
        case 'upper-case#upperCase': {
            return `${prefix}${convertName(value, (s) => (0, upper_case_1.upperCase)(s || ''), transformUnderscore)}`;
        }
        case 'keep':
            return `${prefix}${value}`;
        ...
        default:
            ...
            return `${prefix}${convertName(value, (s) => (0, pascal_case_1.pascalCase)(s || ''), transformUnderscore)}`;
    }
};
const toMockName = (typedName, casedName, prefix) => {
    ...
    const firstWord = (0, sentence_case_1.sentenceCase)(typedName).split(' ')[0];
    return `${(0, indefinite_1.default)(firstWord, { articleOnly: true })}${casedName}`;
};
...
const getNamedType = (opts) => {
    ...
    casual_1.default.seed(hashedString(opts.typeName + opts.fieldName));
...
const plugin = (schema, documents, config) => {
    ...
    const result = (0, plugin_helpers_1.oldVisit)(astNode, { leave: visitor });
